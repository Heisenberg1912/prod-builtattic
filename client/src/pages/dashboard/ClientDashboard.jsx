    ...
    <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            ...
            value={loading ? "GǪ" : catalog.length}
          />
          <StatCard
            icon={<Clock />}
            label="Published"
            value={loading ? "GǪ" : published.length}
          />
          <StatCard
            icon={<ShoppingCart />}
            label="In Cart"
            value={loading ? "GǪ" : cartCount}
          />
          <StatCard
            icon={<DollarSign />}
            label="Catalog Value"
            value={loading ? "GǪ" : formatCurrency(totalValue, currency)}
          />
        </div>
        {error && (
      ...
      <section className="mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
          {loading ? (
            <p className="text-sm text-gray-500">Loading designsGǪ</p>
          ) : featured.length === 0 ? (
  ...
  return (
    <section>
      ...
      <div className="bg-white border rounded-2xl shadow-sm p-6 space-y-4">
        {loading ? (
          <p className="text-sm text-gray-500">Loading ordersGǪ</p>
        ) : error ? (
...
function WishlistView({ catalog, wishlist, setWishlist, loading, currency }) {
  ...
  return (
    <section>
      ...
      {loading ? (
        <p className="text-sm text-gray-500">Loading wishlistGǪ</p>
      ) : wishlist.length === 0 ? (
      ...
      <div className="bg-white border rounded-xl p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          ...
          <div className="rounded-lg border bg-gray-50 p-4">
            ...
            <p className="text-lg font-semibold text-gray-900">
              {loading ? "GǪ" : formatCurrency(totalValue, currency)}
            </p>
          </div>
          <div className="rounded-lg border bg-gray-50 p-4">
